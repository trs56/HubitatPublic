...
When Alarm Out is triggered, the camera switches a dry contact relay on the ouput port from open to closed. This exposes a closed circuit to the Alarm In port and its relay over the jumper wire, which is all it is waiting for in its NO state. When it senses a closed circuit, the event fires. When the Alarm is cleared, the output relay opens and so does the circuit to Alarm In, opening its relay. There is no voltage applied. Voltage is only present when there is a powered sensor (input), siren (output) or security system control panel connected. The reason you don't need to connect the Grounding ports is because both relays share the same path to ground in the camera.
